...
\begin{algorithm}[H]
  ...
  \Return True\;
  \caption{absorptionStateReached($graph$: GraphTool.Graph)}
\end{algorithm}
 \hfill \break
\begin{algorithm}[H]
  ...
  $simulationResult \gets \text{new } SimulationResult(config, rounds)$\;
  \Return $simulationResult$\;
  \caption{runSimulationOn($config$: SimulationConfigurator)}
\end{algorithm}
 \hfill \break
Notare come \emph{updateRule} sia un oggetto ottenuto tramite un'interfaccia che permette di implementare la dinamica di aggiornamento preferita tramite il metodo run. Di seguito l'implementazione di Majority-Dynamics.\\*
...
\begin{algorithm}[H]
  ...
  \caption{Majority-Dynamics.run($graph$: GraphTool.Graph, $v$: GraphTool.Vertex)}
\end{algorithm}
